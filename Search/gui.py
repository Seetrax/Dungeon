...
RED = (255, 0, 0)
GREEN = (0, 255, 0)
args = {'world':os.path.join('mazes','smallmaze.txt'),'frames':10,'omni':True,'player':'agent','method':'a*','graphics' : True}
W=World()
...
def game(args):
    ...
    if (player=='agent'):
        ...
        if args['method']=='a*':
            acts=Search.astar(problem,heuristic=Search.mh)
        ...
        if args['graphics']==True:
            ...
            for i in range(world.num_rows):
                for j in range(world.num_cols):
                    if (j,i) in problem._visitedlist :
                                trans_surface = pygame.Surface((GRID_W, GRID_L), pygame.SRCALPHA)
                                pygame.draw.rect(trans_surface, (255, 255, 0, 128), (0, 0,GRID_W, GRID_L))
